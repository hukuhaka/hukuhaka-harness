...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`/hukuhaka-codex:status ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`/hukuhaka-codex:cancel ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: /hukuhaka-codex:cancel ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: /hukuhaka-codex:result ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push("  Review changes: /hukuhaka-codex:review --wait");
    lines.push("  Stricter review: /hukuhaka-codex:adversarial-review --wait");
  }
...
export function renderCancelReport(job) {
  ...
  lines.push("- Check `/hukuhaka-codex:status` for the updated queue.");
