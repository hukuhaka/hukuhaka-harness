...
- `--enable-stuck-detector` / `--disable-stuck-detector` — after three Bash failures within five minutes, nudge Claude to consider `/hukuhaka-codex:rescue` or `/hukuhaka-codex:duel`. Successful commands are not observed. Detection only; it never invokes Codex on its own.
